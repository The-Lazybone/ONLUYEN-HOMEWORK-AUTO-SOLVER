    ...
    (function () {
        ...
        class WebSearch {
            async search(query) {
                try {
                    ...
                    return data.AbstractText || data.Answer || data.RelatedTopics?.[0]?.Text || data.Definition || "No relevant results found.";
                } catch (error) {
            ...
            shouldSearch(question) {
                ...
                const keywords = ['history', 'date', 'year', 'resolution', 'document', 'event', 'period', 'war', 'battle'];
        ...
        class APIClient {
            async call(prompt, images = []) {
                ...
                const tools = [
                    {
                        ...
                        function: {
                            name: "web_search",
                            description: "Search the web for factual information, historical events, or current data when the question requires external knowledge.",
                            parameters: {
                                type: "object",
                                properties: {
                                    query: {
                ...
                const payload = {
                    model: modelToUse,
                    reasoning_effort: CONFIG.THINK_BEFORE_ANSWER ? "high" : CONFIG.INSTANT_MODE ? "minimal" : "low",
                    messages: [
                        {
                            role: "system",
                            content:
                ...
                if (isSuccess) {
                    ...
                    if (this.failureCount >= CONFIG.RETRIES) {
                        ...
                        if (this.solver.lastApiResponse) {
                            logger.debug("Raw API response at max retries:", JSON.stringify(this.solver.lastApiResponse));
                        }
                        this.failureCount = 0;
                ...
                if (response?.choices?.[0]?.message?.tool_calls) {
                    ...
                    if (toolCall.function.name === 'web_search') {
                        ...
                        const messages = [
                            {
                                role: "system",
                                content: "You are a precise assistant. Use the web_search tool when you need external information to answer accurately. Reply exactly as asked.",
                            },
                            { role: "user", content: userContent },
                        ...
                        if (CONFIG.POLL_KEY) toolHeaders["Authorization"] = `Bearer ${CONFIG.POLL_KEY}`;
                        ...
                        if (!toolRes.ok) {
                            ...
                            throw new Error(`Tool call HTTP ${toolRes.status}: ${txt}`);
                        }
