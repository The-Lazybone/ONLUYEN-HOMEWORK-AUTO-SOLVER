    ...
    // A modular, multi-class implementation for better readability and maintenance.
        ...
        (function() {
            ...
            const CONFIG = {
                ...
                POLL_KEY: (typeof globalThis !== 'undefined' && globalThis.__HW_SOLVER_POLL_KEY__) || (typeof globalThis !== 'undefined' && globalThis.process && globalThis.process.env && globalThis.process.env.HW_SOLVER_POLL_KEY) || '',
            ...
            class Logger {
                ...
                _log(level, ...args) {
                    ...
                    if (this.history.length >= CONFIG.LOG_HISTORY_LIMIT) this.history.shift();
            ...
            class APIClient {
                async call(prompt, images = []) {
                    ...
                    logger.debug(`Using model: ${modelToUse} for prompt with ${images.length} images.`);

                    const payload = {
                        model: modelToUse,
                        messages: [
                            { role: 'system', content: 'You are a precise assistant. Reply exactly as asked.' },
                    ...
                    try {
                        ...
                        logger.debug('Seed generation failed, omitting seed from payload.', e);
                    }
            ...
            class Scraper {
                ...
                _getContainer() { 
                    return document.querySelector('.question-name') 
                ...
                _getCleanedText(element) {
                    ...
                    clone.querySelectorAll('mjx-container').forEach(mjx => {
                        ...
                        if (ariaLabel) {
                            ...
                            // Powers and units
                            cleanedLabel = cleanedLabel.replace(/squared/g, '^2')
                                .replace(/cubed/g, '^3');
                            ...
                            cleanedLabel = cleanedLabel.replace(/StartFraction (.+) Over (.+) EndFraction/g, '($1)/($2)');
                            ...
                            cleanedLabel = cleanedLabel.replace(/(\d+)\s+halves\b/gi, '$1/2');

                            logger.debug('Cleaned MathJax label:', cleanedLabel);
                            mjx.replaceWith(document.createTextNode(` ${cleanedLabel} `));
                        } else {
                ...
                scrapeMCQ() {
                    ...
                    if (!container) return { type: 'mcq', question: '', options: [], images: [] };
                    ...
                    const nodes = Array.from(document.querySelectorAll('.row.text-left.options .question-option'));
                    const options = nodes.map(node => ({
                        letter: node.querySelector('.question-option-label')?.innerText.trim() || null,
                        text: this._getCleanedText(node.querySelector('.question-option-content')),
                        images: this._scrapeImages(node.querySelector('.question-option-content')),
                        element: node,
                    })).filter(o => o.letter);
                ...
                scrapeFillable() {
                    const container = this._getContainer();
                    if (!container) return { type: 'fillable', question: '', blanks: [], images: [] };
                    ...
                    if (qNode) {
                        ...
                        clone.querySelectorAll("input[type='text'], textarea").forEach(el => {
                            ...
                            const ansSpan = parent.querySelector('.ans-span-second');
                            ansSpan?.remove();
                            // Replace input with [BLANK]
                            parent.replaceChild(document.createTextNode(' [BLANK] '), el);
                        });
                ...
                scrapeTrueFalse() {
                    const container = this._getContainer();
                    if (!container) return { type: 'truefalse', question: '', subQuestions: [], images: [], table: null };
                    ...
                    const subQuestionNodes = Array.from(document.querySelectorAll('.question-child .child-content'));
                    const subQuestions = subQuestionNodes.map(node => ({
                        char: node.querySelector('.option-char')?.innerText.trim().replace(')', '').replace('.', '') || null,
                    ...
                    return { type: 'truefalse', question: questionText, subQuestions, images, table };
                }
            }
            ...
            class UIController {
                _simulateClick(el) {
                    if (!el) return;
                    try { el.scrollIntoView({ block: 'center', inline: 'center' }); } catch (e) { logger.debug('Scroll into view failed:', e); }
                    try { el.focus?.(); } catch (e) { logger.debug('Focus failed:', e); }
                    try { el.click(); } catch (e) { logger.debug('Click failed:', e); }
                    try {
                        el.dispatchEvent(new Event('input', { bubbles: true }));
                        el.dispatchEvent(new Event('change', { bubbles: true }));
                    } catch (e) { logger.debug('Dispatch event failed:', e); }
                ...
                async _simulateTyping(element, text) {
                    ...
                    element.dispatchEvent(new MouseEvent('mouseenter', { bubbles: true, composed: true }));
                    ...
                    for (let i = 0; i < text.length; i++) {
                        ...
                        element.dispatchEvent(new KeyboardEvent('keydown', { key: char, keyCode: keyCode, charCode: charCode, bubbles: true, composed: true }));
                        element.dispatchEvent(new KeyboardEvent('keypress', { key: char, keyCode: keyCode, charCode: charCode, bubbles: true, composed: true }));

                        element.value += char;
                        element.dispatchEvent(new InputEvent('input', { data: char, inputType: 'insertText', bubbles: true, composed: true }));

                        element.dispatchEvent(new KeyboardEvent('keyup', { key: char, keyCode: keyCode, charCode: charCode, bubbles: true, composed: true }));
                    ...
                    element.blur();
                    const logMode = CONFIG.INSTANT_MODE ? 'fast typing' : 'human typing';
            ...
            _scheduleNext(isSuccess) {
                ...
                if (isSuccess) {
                    ...
                    if (this.failureCount >= CONFIG.RETRIES) {
                        logger.warn(`Max retries (${CONFIG.RETRIES}) reached for current question. Skipping to next.`);
                ...
                if (this.failureCount > 0) {
                    backoff = Math.min(1000 * Math.pow(2, this.failureCount - 1), 30000);
                }
            ...
            class HomeworkSolver {
                ...
                _buildMCQPrompt(question, options) {
                    ...
                    let p = thinkPrefix + 'You are a homework solver. Reply with a single uppercase letter: A, B, C or D — nothing else.\n\n';
                    p += `Question:\n${question}\n\nChoices:\n`;
                    options.forEach(opt => {
                ...
                return thinkPrefix + `You are a homework solver. If the question is in another language, translate it to English first and then solve it step by step. Fill the blank(s) with short phrase(s) or word(s) or a number. For numerical answers, use a comma (,) as the decimal separator. Reply only with the short answer (numerical if possible), with no prefixes or suffixes. \n\nQuestion:\n${question}`;
            }
                ...
                _buildTrueFalsePrompt(question, subQuestions, table) {
                    ...
                    let p = thinkPrefix + 'You are a homework solver. For each sub-question, reply with "TRUE" or "FALSE" only, separated by commas. Example: TRUE,FALSE,TRUE,TRUE\n\n';
                    p += `Main Question:\n${question}\n\n`;
                    if (table) {
                        p += `Table Data:\n${table}\n\n`;
                    }
            ...
            _parseFill(response) {
                ...
                let text = response?.choices?.[0]?.message?.content || response?.answer || (typeof response === 'string' ? response : '');
                ...
                _parseTrueFalse(response, subQuestions) {
                    ...
                    for (let i = 0; i < subQuestions.length; i++) {
                        ...
                        if (answer === 'TRUE') {
                            ...
                            parsedResults.push({ char: subQuestions[i].char, value: null }); // Indicate parsing failure
                        }
                    }
                ...
                async _solveMCQ({ question, options, images }) {
                    ...
                    const optionToSelect = options.find(o => o.letter.toUpperCase() === letter);
                    if (!optionToSelect) {
                        logger.warn(`LLM answered '${letter}', but no such option was found.`);
                        return false;
                    }
                ...
                async _solveTrueFalse({ question, subQuestions, images, table }) {
                    ...
                    logger.info(`LLM response parsed to: '${JSON.stringify(parsedAnswers)}'`);

                    if (parsedAnswers.length !== subQuestions.length || parsedAnswers.some(a => a.value === null)) {
                        logger.warn('Could not determine all True/False answers from LLM.');
                        return false;
                    }
            ...
            window.hwSolver = {
                ...
                help: () => {
                    ...
                    console.log('  hwSolver.toggleInstantMode() — toggle fast typing mode (instant vs human-like)');
                    console.log('  hwSolver.toggleThinkBeforeAnswer() — toggle reasoning-before-answer (FINAL: outputs)');
                    console.log('  hwSolver.config            — read/write runtime configuration');
                    console.log('  hwSolver.logger.history    — view recent log entries');
